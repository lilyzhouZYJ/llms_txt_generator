...
A web app that crawls a website and generates a spec-compliant [llms.txt](https://llmstxt.org/) file. It uses OpenAI to enrich page metadata and infer site structure, with a rule-based fallback when the API is unavailable.

## How it works

1. Crawls the site (BFS, up to 100 pages)
2. Extracts metadata from each page (title, description, section hints)
3. Calls OpenAI to generate descriptions, a site summary, and section groupings
4. Outputs a formatted `llms.txt` Markdown file
